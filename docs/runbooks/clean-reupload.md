...
```bash
vcfa_register_host       # VCFA:Host — 모든 $data 드롭다운(projects/namespaces/vmclass/vmimage/krversion/contentslibrary/storageclass) CCI 소스
...
# vcfa_register_vcenter  # (선택/레거시) getStorageClass 도 CCI 전환(2026-06-26). 진단 dumpVcRoots 용만.
...
| vCenter 미등록 | (영향 없음 — getStorageClass 도 CCI. 진단 dumpVcRoots 만 vCenter 필요) |
